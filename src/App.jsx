...
// Yahoo Finance direct fetch with no-cors workaround
async function fetchYahoo(url) {
  // Try proxies in order
  for (const proxy of PROXIES) {
    try {
      const proxyUrl = proxy.includes("cors.sh") 
        ? proxy + url
        : proxy + encodeURIComponent(url);
      const r = await fetch(proxyUrl, {
        signal: AbortSignal.timeout(10000),
        headers: proxy.includes("cors.sh") ? {"x-cors-api-key": "temp_" + Date.now()} : {}
      });
      if (r.ok) {
...
async function fetchAllCharts(range) {
...
export default function App() {
  ...
  const loadCharts = useCallback(async (range) => {
    ...
    try {
      ...
      const fresh = await fetchAllCharts(range);
      setHistData(fresh);
    } catch {}
  ...
  const refreshAll = useCallback(async () => {
    ...
    setLoading(false);
    // Charts load in background after quotes
