...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Source Sans 3','Source Sans Pro','Helvetica Neue',Arial,sans-serif",background:"#F4F2EE",minHeight:"100vh"}}>
      ...
      {(Object.keys(histData).length > 0 || chartLoading) && (
        <div style={{background:"#FFF",borderBottom:"1px solid #E0DDD8",padding:"24px 28px"}}>
          <div style={{maxWidth:1440,margin:"0 auto"}}>
            {/* Time range selector */}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <span style={{fontSize:13,fontWeight:700,color:"#0B2447"}}>Portfolio Balance</span>
            ...
            <div style={{marginBottom:24}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
                <span style={{fontSize:11,color:"#999",fontWeight:600,letterSpacing:"0.08em"}}>ALL ACCOUNTS</span>
                <span style={{fontSize:18,fontWeight:700,color:"#0B2447"}}>{fC(totalVal)}</span>
              </div>
              <PortfolioChart data={buildPortfolioHistory("ALL")} color="all" height={160}/>
            </div>

            {/* Individual account charts */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
              {ACCT_ORDER.map(acct=>{
                const acctRows = withW.filter(r=>r.account===acct);
                const acctVal = acctRows.reduce((s,r)=>s+(r.mv??r.cb),0);
                const chartData = buildPortfolioHistory(acct);
                return (
                  <div key={acct} style={{background:"#FAFAF8",borderRadius:8,padding:"14px 16px",border:"1px solid #EEE",borderTop:`3px solid ${ACCT_COLOR[acct]}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
                      <span style={{fontSize:10,fontWeight:800,color:ACCT_COLOR[acct],letterSpacing:"0.1em"}}>{acct}</span>
                      <span style={{fontSize:13,fontWeight:700,color:"#0B2447"}}>{fC(acctVal)}</span>
                    </div>
                    <PortfolioChart data={chartData} color={acct} height={70}/>
                  </div>
                );
              })}
            </div>
